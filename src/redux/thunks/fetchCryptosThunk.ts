...
export const fetchCoinData =
  (start: number, limit: number): ThunkAction<void, AppState, undefined, Action<string>> =>
  async (dispatch) => {
    dispatch(fetchCoinDataRequest())
    try {
      const response = await fetch(
        `https://api.coincap.io/v2/assets?offset=${start}&limit=${limit}`,
      )
